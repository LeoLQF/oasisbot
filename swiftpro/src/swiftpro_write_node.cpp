...
void position_write_callback(const swiftpro::position& msg)
{
	...
	sprintf(y, "%.2f", msg.y);
	sprintf(z, "%.2f", msg.z);
	sprintf(z_, "%.2f", msg.z + 50);
	Gcode = (std::string)"G0 X" + x + " Y" + y + " Z" + z_ + " F50" + "\r\n";
	_serial.write(Gcode.c_str());
//	 while(_serial.read(_serial.available()) == "ok\n");
	Gcode = (std::string)"G0 X" + x + " Y" + y + " Z" + z + " F50" + "\r\n";
...
{	
	...
	ros::Publisher 	pub_arrived = nh.advertise<std_msgs::Bool>("Arrived", 1);
